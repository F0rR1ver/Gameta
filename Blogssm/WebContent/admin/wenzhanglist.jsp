...
<head>
    ...
    <link href="<%=path %>/assetsqt/zoom.css" rel="stylesheet">
  <script src="<%=path %>/assetsqt/transition.js"></script>
    <script src="<%=path %>/assetsqt/zoom.js"></script>
                        ...
                        <div class="c_panel">
                            ...
                            <div class="c_content">
                                ...
                                <div class="margin-bottom-20">
                                    <div class="btn-group">
                                     <form action="wenzhanglist.do" method="post">
                                    <div class="input-group" style="margin-left: 400px;">
                                      <input type="text" class="form-control" style="width: 200px;" name="key" value="${key }" placeholder="请输入标题...">
                                        <select name="key1" class=" form-control" style="width: 200px;" >
                                  ...
                                  <table class="table table-hover dataTable table table-striped table-bordered" >
                                    ...
                                    <c:forEach items="${pageInfo.list }" var="wenzhang">
                                      ...
                                      <tr align="center">
                                        <td>
                                        <img alt="" src="<%=path %>/upload/${wenzhang.filename }" style="width: 60px;height: 30px;" data-action="zoom">
                                        </td>
                                        <td>${wenzhang.member.tname }</td>
                                        <td>${wenzhang.title }</td>
                                        <td>${wenzhang.type.name }</td>
                                        <td>${wenzhang.savetime }</td>
                                        <td>
                                        <a href="liuyanlist.do?xwid=${wenzhang.id }" style="color: red;">查看</a>
                                        </td>
                                        <td>${wenzhang.shstatus }</td>
                                        <td >
                                        <c:if test="${wenzhang.shstatus eq '待审核' }">
                                         <a href="wenzhangshTG.do?id=${wenzhang.id }"  >通过</a> 
                                         <a href="wenzhangshJJ.do?id=${wenzhang.id }"  >未通过</a> 
                                         
                                         </c:if>
                                         <a href="wenzhangDel.do?id=${wenzhang.id }"  title="删除" ><i class="fa fa-trash"></i></a>
                                        </td>
                                      </tr>
                                        </c:forEach>
                                        ...
                                        <tr>
                                            <td colspan="8">
                                                ...
                                                <div id="pagediv" style="border:0px solid red;">
                                                    <ul>
                                                        <li><a href="wenzhanglist.do?pageNum=${pageInfo.hasPreviousPage==false?1:pageInfo.prePage}">上一页</a></li>
                                                        <c:forEach begin="1" end="${pageInfo.pages}" varStatus="status">
                                                            <li class="pagesz ${status.count eq pageInfo.pageNum ?"acvtive":""}" ><a href="wenzhanglist.do?pageNum=${status.count}" >${status.count}</a></li>
                                                        </c:forEach>
                                                        <li><a href="wenzhanglist.do?pageNum=${pageInfo.hasNextPage==false? pageInfo.pages : pageInfo.nextPage}">下一页</a></li>
                                                    </ul>
                                                </div>
                                                </c:if>
                                            </td>
